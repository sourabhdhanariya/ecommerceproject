...
<div class="pcoded-content">
   ...
   // $sql = "SELECT * FROM `customer_order` WHERE 1 ";
                                        ...
                                        if ($selectedFilter === 'completed') {
                                            $sql .= " WHERE status = 1"; // Add the WHERE clause here
                                        } elseif ($selectedFilter === 'pending') {
                                            $sql .= " WHERE status = 0"; // Add the WHERE clause here
                                        } elseif ($selectedFilter === 'cancelled') {
                                            $sql .= " WHERE status = 2"; // Add the WHERE clause here
                                        }
                                        
                                        if ($selectedTime !== 'all') {
                                            // Add the appropriate date range condition here
                                            // For example: $sql .= " AND order_date BETWEEN 'start_date' AND 'end_date'";
                                        }
                                        ...
                                        ?>
                                                ...
                                                <tr class="order-row" data-status="<?= $status ?>" data-order-date="<?= $order_date ?>">
                                                    <td><?= $counter ?></td>
                                                    <td>
                                                        <button type="button" class="btn btn-link btn-sm view-btn" data-toggle="modal" data-product-id="<?= $id ?>" data-product-name="<?= $product_name ?>" data-price="<?= $price ?>" data-customerName="<?= $customername ?>" data-biilingAddress="<?= $customer_address ?> <?= $city ?> <?= $state ?> <?= $country ?> <?= $zip ?>" ; data-status="<?= $status ?>" data-shipingAddress="<?= $shiping_address ?> <?= $shiping_city ?> <?= $shiping_state ?> <?= $shiping_country ?> <?= $shiping_zip ?>" ; data-image="images/<?= $image ?>" data-order="<?= $order_id ?>" data-customer-order="<?= $order_date ?>" data-quantity="<?= $quantity ?>" data-category="<?= $category ?>" data-target="#modelId">
                                                            <?= $order_id ?>
                                                        </button>
                                                    </td>
